...
class ChatViewController: JSQMessagesViewController {
    ...
    override func viewDidLoad() {
        ...
        if let ImageData = UserDefaults.standard(forKey: "backGroundImage"){
            
            if (imageData as AnyObject).lenght != 0{
    ...
    override func didPressSend(_ button: UIButton!, withMessageText text: String!, senderId: String!, senderDisplayName: String!, date: Date!) {
        ...
        let rootRef = FIRDatabase.database().reference(fromURL: "https://gokinjyosns.firebaseio.com/").child("message").child(address).child(roomName)
     ...
     //Firebaseからメッセージを取得する
    func getInfo(){
        
        UIApplication.shared.isNetworkActivityIndicatorVisible = true
        let firebase = FIRDatabase.database().reference(fromURL: "https://gokinjyosns.firebaseio.com/").child("message").child(address).child(roomName)
